...
const LUANDA_CENTER: [number, number] = [13.234, -8.839];

const BUS_ROUTES = [
  {
    id: 'bus-1',
    name: 'TC Linha 01',
    subtitle: 'Maculusso ↔ Baixa',
    color: '#22c55e',
    type: 'bus' as const,
    status: 'active',
    frequency: '15 min',
...
const AI_PREDICTIONS = [
  {
    ...
    location: 'Av. Deolinda Rodrigues',
    time: '17:00 – 19:00',
    confidence: 87,
    severity: 'high' as const,
    description:
      'Pico de congestionamento previsto para hora de saída do trabalho',
  },
  {
    id: 'pred-2',
    type: 'landslide' as const,
    location: 'Estrada Benfica–Camama',
    time: '14:00 – 16:00',
    confidence: 72,
    severity: 'medium' as const,
    description:
      'Risco de deslizamento após chuvas fortes previstas pela meteorologia',
  },
  {
    id: 'pred-3',
    type: 'congestion' as const,
    location: 'Av. Revolução de Outubro',
    time: '08:00 – 09:30',
    confidence: 91,
    severity: 'medium' as const,
    description:
...
const LUANDA_LOCATIONS = [
  {
    name: 'Terminal do Maculusso',
    coords: [13.238, -8.823] as [number, number],
  },
  { name: 'Baixa de Luanda', coords: [13.234, -8.839] as [number, number] },
  { name: 'Ingombotas', coords: [13.254, -8.822] as [number, number] },
  { name: 'Maianga', coords: [13.263, -8.811] as [number, number] },
  { name: 'Kinaxixi', coords: [13.225, -8.83] as [number, number] },
  { name: 'Viana', coords: [13.341, -8.912] as [number, number] },
];
...
const BOT_FLOW: Record<string, string> = {
  ...
  done: '✅ Reporte enviado com sucesso! Vou alertar as autoridades competentes e a comunidade. Obrigado por contribuir para uma melhor mobilidade em Luanda.',
};
...
function RouteCard({
  ...
  const handleFromChange = (val: string) => {
    const loc = LUANDA_LOCATIONS.find((l) => l.name === val);
    if (loc) {
      setFrom(val);
    ...
    <>
      ...
      {activeRoute && activeRoute.coords.length >= 2 && (
        <>
          ...
          <MapMarker
            longitude={activeRoute.coords[activeRoute.coords.length - 1][0]}
            latitude={activeRoute.coords[activeRoute.coords.length - 1][1]}
          >
            <MarkerContent>
  ...
  const initialMessages: Message[] = [
    {
      ...
      role: 'bot',
      content:
        'Olá! Sou o assistente TransportConnect 🤖\n\nEstou aqui para recolher reportes de incidentes em Luanda. Selecione o tipo de ocorrência abaixo para começar.',
    },
  ];
  ...
  return (
    <>
      <div className="h-screen w-full relative overflow-hidden">
        ...
        {/* Map */}
        <Map center={LUANDA_CENTER} zoom={13}>
          <MapController />
          <MapLegend />
        </Map>
      </div>
